...
async def export_user_pdf(user_id: str):
    try:
        pdf_file = generate_user_pdf(user_id)
        if pdf_file:
            return StreamingResponse(
                pdf_file, 
                media_type="application/pdf",
                headers={"Content-Disposition": f"attachment; filename=rapor_kullanici_{user_id}.pdf"}
            )
        else:
            return JSONResponse(content={"error": "PDF oluşturulamadı"}, status_code=500)
    except Exception as e:
        print(e)
        return JSONResponse(content={"error": "PDF çıktısı alınamadı"}, status_code=500)
